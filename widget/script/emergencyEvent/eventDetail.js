  ...
  var vm = new Vue({
    ...
    methods: {
      ...
      subButtonClick: function () {
        ...
        if(this.imgarr2.imgpaths.length<2){
          alert('必须上传至少两张图片');
          return false;
        }
        // alert(JSON.stringify(this.imgarr2));
      ...
      uploadAttach: function (cb) {
        ...
        if (imgpaths.length > 0) {
          ...
          api.ajax({
            url: UICore.serviceUrl + 'pc_mobile/emergency_event/emergency_upload_attach?act=uploadAttach_HZ&loginId=' + this.accountId+'&accountId=' +this.accountId + '&accountName=' + this.f.approvePersonName + '&workId=' + this.alltag.flowInstanceId + '&nodeId=' + this.alltag.activityId,
            method: 'post',
            tag: 'grid',
            data: {
              files: {

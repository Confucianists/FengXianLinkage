window.apiready = function() {
    ...
    var vm = new Vue({
        ...
        methods: {
            ...
            getForm: function() {
                var this_ = this;
                console.log(UICore.serviceUrl + 'mobile/mobileWf.shtml?act=editEvent_HZ&state=VIEW&accountId=' + this.accountId + '&workId=' + this.id);
                api.ajax({
                    url: UICore.serviceUrl + 'mobile/mobileWf.shtml?act=editEvent_HZ&state=VIEW&accountId=' + this.accountId + '&workId=' + this.id,
                    tag: 'grid',
                    method: 'get'
                }, function(ret, err) {
                    if (ret) {
                        ...
                        for (var column in data) {
                            if (data[column].column_label == "hidden") {
                                ...
                                if (data[column].column_name == "limit" && data[column].default_value == "2") {
                                    this_.show.Process = true
                                }
            ...
            openProcess: function() {
                var this_ = this;
                UICore.openSelect3(this.Process_arr, this.limitFlag, "process");
                api.addEventListener({
                    name: 'process'
                }, function(ret, err) {
                    if (ret) {
                        this_.limit = ret.value.key1;
                        this_.alltag.limitFlag=ret.value.key2;
                    } else {
                        alert(JSON.stringify(err));
                    }
                });
            ...
            submitFormNew: function() {
                ...
                api.ajax({
                    ...
                    data: {
                        values: {
                            "data": JSON.stringify(_this.alltag)
                        }
                    }
                }, function(ret, err) {
                    api.hideProgress();
